...
public final class GizmoDragMath {
    ...
    public static Vector3d translatedModelPosition(Vector3dc startingModelPosition, Vector3dc axis,
        ...
        java.util.Objects.requireNonNull(startingModelPosition, "startingModelPosition");
    ...
    /** Keeps tangential rotation sensitivity stable when the ring grows with the selection bounds. */
    public static float scaleIndependentRotationDegrees(float rawDegrees, double rotationRadius) {
        if (!Float.isFinite(rawDegrees)) {
            throw new IllegalArgumentException("rawDegrees must be finite");
        }
    ...
    /** Screen-distance ratio used by a uniform scale handle. */
