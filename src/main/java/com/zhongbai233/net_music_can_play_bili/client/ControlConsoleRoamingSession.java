...
public final class ControlConsoleRoamingSession {
    ...
    public static void tick() {
        ...
        Vec3 next = session.camera.getPosition(1.0F).add(session.velocity);
    ...
    public static void suppressPlayerInput(Input input) {
        if (active == null) {
            return;
        }
        // 1.21.1 的 Input 直接暴露冲量/布尔字段（无 keyPresses 集合）：
        // 逐字段清零等价于 26.x 的 keyPresses = Input.EMPTY + setMoveVector(Vec2.ZERO)。
    ...
    public static boolean handlePlacementKey(int key, int action) {
        ...
        Vec3 camera = session.camera.getPosition(1.0F);
    ...
    public static void drawHud(GuiGraphics graphics) {
        Session session = active;
        Minecraft minecraft = Minecraft.getInstance();
        ...
        graphics.drawCenteredString(minecraft.font, mode, cx, cy + 18, color);
    }

    public static void submitGeometry(PortSubmitNodeCollector collector) {
        Session session = active;
        Minecraft minecraft = Minecraft.getInstance();
        ...
        box(lines, ox - (float) session.rangeX, oy - (float) session.rangeY, oz - (float) session.rangeZ,
        ...
        for (int i = 0; i < session.elements.size(); i++) {
            ...
            if (element.isAudio()) {
                orientedBox(lines, cx, cy, cz, xAxis, yAxis, zAxis, halfW, halfH,
                ...
                line(lines, p0.x, p0.y, p0.z, p1.x, p1.y, p1.z, color, 2.0F);
                line(lines, p1.x, p1.y, p1.z, p2.x, p2.y, p2.z, color, 2.0F);
                line(lines, p2.x, p2.y, p2.z, p3.x, p3.y, p3.z, color, 2.0F);
                line(lines, p3.x, p3.y, p3.z, p0.x, p0.y, p0.z, color, 2.0F);
            }
        }
        submitWidthGroupedLines(collector, lines);
    }

    /** 1.21.1 无逐顶点线宽;按线宽分组、每组一个带 LineStateShard 的 RenderType 批次提交。 */
    private static void submitWidthGroupedLines(PortSubmitNodeCollector collector, List<DebugLine> lines) {
        if (lines.isEmpty()) {
            return;
        }
                    ...
                    (pose, buffer) -> {
                        for (DebugLine debugLine : group) {
                            buffer.addVertex(pose, debugLine.x1, debugLine.y1, debugLine.z1)
                                    .setColor(debugLine.color).setNormal(0.0F, 1.0F, 0.0F);
    ...
    private record DebugLine(float x1, float y1, float z1, float x2, float y2, float z2, int color, float width) {
    }
    ...
    private static void orientedBox(List<DebugLine> lines, float cx, float cy, float cz,
        ...
        for (int[] edge : edges) {
            Vector3f a = p[edge[0]];
            Vector3f b = p[edge[1]];
            line(lines, a.x, a.y, a.z, b.x, b.y, b.z, color, lineWidth);
        }
    }

    private static void box(List<DebugLine> lines, float minX, float minY, float minZ,
            float maxX, float maxY, float maxZ, int color, float lineWidth) {
        line(lines, minX, minY, minZ, maxX, minY, minZ, color, lineWidth);
        line(lines, maxX, minY, minZ, maxX, minY, maxZ, color, lineWidth);
        line(lines, maxX, minY, maxZ, minX, minY, maxZ, color, lineWidth);
        line(lines, minX, minY, maxZ, minX, minY, minZ, color, lineWidth);
        line(lines, minX, maxY, minZ, maxX, maxY, minZ, color, lineWidth);
        line(lines, maxX, maxY, minZ, maxX, maxY, maxZ, color, lineWidth);
        line(lines, maxX, maxY, maxZ, minX, maxY, maxZ, color, lineWidth);
        line(lines, minX, maxY, maxZ, minX, maxY, minZ, color, lineWidth);
        line(lines, minX, minY, minZ, minX, maxY, minZ, color, lineWidth);
        line(lines, maxX, minY, minZ, maxX, maxY, minZ, color, lineWidth);
        line(lines, maxX, minY, maxZ, maxX, maxY, maxZ, color, lineWidth);
        line(lines, minX, minY, maxZ, minX, maxY, maxZ, color, lineWidth);
    }

    private static void line(List<DebugLine> lines, float x1, float y1, float z1,
            float x2, float y2, float z2, int color, float lineWidth) {
        lines.add(new DebugLine(x1, y1, z1, x2, y2, z2, color, lineWidth));
    }
