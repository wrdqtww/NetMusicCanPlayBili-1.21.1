...
public final class ClientMP4QueueTooltip implements ClientTooltipComponent {
    ...
    public void renderImage(Font font, int x, int y, GuiGraphics graphics) {
        ...
        graphics.renderOutline(x, y, width, componentHeight, 0xFF6D4AA2);
        graphics.drawString(font, "MP4 播放队列", x + PADDING_X, y + PADDING_Y, 0xFFBBD8FF);
        graphics.drawString(font, tooltip.titles().size() + " 首",
        ...
        for (int row = 0; row < rows; row++) {
            ...
            if (active) {
                ...
                graphics.renderOutline(x + 4, rowY - 1, width - 8, ROW_HEIGHT, 0xFF74C7FF);
            }
            graphics.drawString(font, active ? "▶" : "•", x + PADDING_X, rowY, active ? 0xFF74C7FF : 0xFF59647C);
            graphics.drawString(font, displayTitle(tooltip.titles().get(index)), x + PADDING_X + 13, rowY,
