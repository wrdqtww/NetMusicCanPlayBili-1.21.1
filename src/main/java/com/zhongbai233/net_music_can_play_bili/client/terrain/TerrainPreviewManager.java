    ...
    private static final class Session {
        ...
        private void capture(TerrainSectionKey key) {
            ...
            for (int localY = 0; localY < TerrainSectionKey.SIZE; localY++) {
                ...
                if (worldY < bounds.minY() || worldY > bounds.maxY()
                        || worldY < level.getMinBuildHeight() || worldY >= level.getMaxBuildHeight()) {
                    continue;
                }
        ...
        private CapturedNeighborhood captureNeighborhood(TerrainSectionKey key) {
            ...
            for (int localY = TerrainNeighborhoodIndex.MIN_LOCAL;
                    localY <= TerrainNeighborhoodIndex.MAX_LOCAL; localY++) {
                int worldY = key.minBlockY() + localY;
                boolean validY = worldY >= level.getMinBuildHeight() && worldY < level.getMaxBuildHeight()
        ...
        private List<Integer> safeTintLayers(BlockState state) {
            try {
                ...
                // 1.21.1 无 getTintSources:按 tintIndex 顺序枚举注册层,首个未注册索引即终止。
                for (int index = 0; index < 32; index++) {
                    try {
                        int color = blockColors.getColor(state, level, cursor, index);
                        if (color == -1) {
                            break;
                        }
                        colors.add(color);
        ...
        private static TerrainBlockEntityPreview extractBlockEntityPreview(
            ...
            // 1.21.1 无 BlockEntityRenderState 提取:仅校验存在渲染器,直接存活的方块实体,
            // 由 PIP 在渲染线程经由 BlockEntityRenderer.render 直接提交。
            if (dispatcher.getRenderer(blockEntity) == null) {
                return null;
            }
            return new TerrainBlockEntityPreview(blockEntity.getBlockPos(), blockEntity);
        }
