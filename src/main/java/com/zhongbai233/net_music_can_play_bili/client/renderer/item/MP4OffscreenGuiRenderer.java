...
final class MP4OffscreenGuiRenderer implements AutoCloseable {
    ...
    private void withLandscape(GuiGraphics g, Runnable draw) {
        g.pose().pushPose();
        g.pose().translate(0.0F, (float) HEIGHT, 0.0F);
        g.pose().mulPose(Axis.ZP.rotation((float) -Math.PI / 2.0F));
        draw.run();
        g.pose().popPose();
    }
    ...
    private void marqueeText(GuiGraphics g, int x, int y, int maxWidth, String text, int color,
            boolean small) {
        Font font = Minecraft.getInstance().font;
        float scale = 1.0F;
    ...
    private void centeredMarqueeText(GuiGraphics g, int centerX, int y, int maxWidth, String text,
    ...
    private void steppedGradientV(GuiGraphics g, int x, int y, int w, int h, int topColor, int bottomColor,
            int steps) {
    ...
    private void drawWrappedCentered(GuiGraphics g, int centerX, int centerY, int maxWidth,
