...
public final class HolographicGlassesWorldScreenRenderer {
    ...
    public static void onSubmitCustomGeometry(RenderLevelStageEvent event) {
        if (!ENABLED || !PortWorldRenderEvents.isStage(event, RenderLevelStageEvent.Stage.AFTER_TRANSLUCENT_BLOCKS)) {
            return;
        }
    ...
    private static void submitBinding(PortSubmitNodeCollector collector, MediaSource source, ScreenQuad quad,
    ...
    private static void submitMediaDevice(PortSubmitNodeCollector collector, UUID deviceId, ScreenQuad quad,
            boolean rgbaFallback) {
        if (deviceId == null) {
            return;
        }
        ...
        if (rgbaFallback) {
            ...
            collector.submitCustomGeometry(poseStack, RenderType.entityCutout(textureId),
    ...
    private static void submitProjector(PortSubmitNodeCollector collector, MediaSource source, ScreenQuad quad,
            boolean rgbaFallback) {
    ...
    private static void submitTurntable(PortSubmitNodeCollector collector, MediaSource source, ScreenQuad quad,
            boolean rgbaFallback) {
        VideoBillboardPreview.ProjectorFrameSnapshot frame = VideoBillboardPreview.currentTurntableFrame(source.pos());
        submitFrame(collector, frame, quad, rgbaFallback);
    }

    private static void submitFrame(PortSubmitNodeCollector collector, VideoBillboardPreview.ProjectorFrameSnapshot frame,
        ...
        if (!rgbaFallback && frame.yuv()) {
            ...
            collector.submitCustomGeometry(poseStack,
                    frame.emissiveRgba()
                            ? RenderType.entityCutout(frame.rgbaTexture())
