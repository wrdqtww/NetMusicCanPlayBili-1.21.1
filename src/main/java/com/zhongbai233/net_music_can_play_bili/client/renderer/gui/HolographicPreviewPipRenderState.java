...
/**
 * Frozen per-frame snapshot consumed by {@link HolographicPreviewPipRenderer}.
 *
 * <p>1.21.1 has no {@code EntityRenderState}/{@code PictureInPictureRenderState}: the live
 * {@link LivingEntity} is captured instead, and the renderer applies the inventory-style
 * pose adjustments (body/head rotation, shadow off) around the draw call. The original
 * pip-rect fields (x0/y0/x1/y1 + scale) are preserved as the framebuffer sizing contract.</p>
 */
public record HolographicPreviewPipRenderState(LivingEntity playerEntity, Vector3f playerTranslation,
    ...
    public HolographicPreviewPipRenderState(LivingEntity playerEntity, Vector3f playerTranslation,
            float playerScale, float previewYaw, float previewPitch, boolean firstPerson, float fovDegrees,
            boolean playerGlowing, float screenDistance, float screenOffsetX, float screenOffsetY,
            float screenHeight, float screenAspect, float screenRoll, int gizmoTool, int gizmoHandle,
            boolean localSpace,
            int x0, int y0, int x1, int y1, float scale) {
        this(playerEntity, playerTranslation, playerScale, previewYaw, previewPitch, firstPerson, fovDegrees,
                playerGlowing,
