...
final class Nv12PboUploader implements AutoCloseable {
    ...
    private boolean upload(AbstractTexture texture, int width, int height, int format, int type, int bytes,
            BufferWriter writer) {
        if (texture == null) {
            return false;
        }
        ...
        try {
            ...
            LOGGER.warn("NV12/PBO: {} 上传失败，将回退 glTexSubImage2D 普通上传: {}", label, e.toString());
            return false;
        } finally {
            if (mappedBuffer) {
                try {
