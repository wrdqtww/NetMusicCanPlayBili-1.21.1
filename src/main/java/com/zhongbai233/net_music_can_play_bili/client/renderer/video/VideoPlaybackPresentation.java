...
final class VideoPlaybackPresentation {
    ...
    void submit(PortSubmitNodeCollector collector, Minecraft minecraft, Camera camera) {
        ...
        for (BlockPos pos : projectorPositions) {
            ...
            if (VideoBerConsumerVisibilityPolicy.usesBerSubmission(berManaged, submittedByBer)) {
                ...
                boolean predicted = com.zhongbai233.net_music_can_play_bili.client.renderer.ControlConsoleRenderer
                        ...
                        || minecraft.level.getBlockEntity(pos) instanceof VideoProjectorBlockEntity projector
                            && isPredictedProjectorVisibleSoon(minecraft, projector);
            ...
            boolean projectorPrewarm = projectorRenderable
                    || isPredictedProjectorVisibleSoon(minecraft, projector);
        ...
        for (VideoProjectorBlockEntity projector : renderableProjectors) {
            submitProjector(collector, minecraft, camera, projector);
        }
    }

    private void submitProjector(PortSubmitNodeCollector collector, Minecraft minecraft, Camera camera,
        ...
        if (HolographicGlassesClient.shouldHideProjectorVideos()) {
            ...
            VideoBillboardPreview.submitProjectorEmissiveGeometry(collector, minecraft, camera, projector,
            ...
            VideoBillboardPreview.submitProjectorGeometry(collector, minecraft, camera, projector,
            ...
            VideoBillboardPreview.submitProjectorYuvGeometry(collector, minecraft, camera, projector,
                    owner.textures.yuvTextureSet());
        } else if (VideoPipelineProperties.loadingPlaceholderEnabled()) {
            submitLoadingPlaceholder(collector, minecraft, camera, projector);
        }
    }

    private void submitLoadingPlaceholder(PortSubmitNodeCollector collector, Minecraft minecraft, Camera camera,
            VideoProjectorBlockEntity projector) {
        ...
        if (kind == VideoPlaceholderFrames.Kind.IRIS_WARNING && VideoPlaceholderFrames.IRIS_VIEW_DEPTH_OFFSET > 0.0D) {
            VideoBillboardPreview.submitProjectorViewDepthOffsetGeometry(collector, minecraft, camera, projector,
    ...
    private static boolean isPredictedProjectorVisibleSoon(Minecraft minecraft,
            VideoProjectorBlockEntity projector) {
        return minecraft != null
                && VideoBillboardPreview.isProjectorScreenPredictedVisible(
                        minecraft, minecraft.gameRenderer.getMainCamera(), projector);
    }
