...
final class PadMapGuiLayer {
    ...
    private void blitClippedMap(GuiGraphics g, ResourceLocation texture, PadMapProjection.Viewport viewport,
        ...
        // 1.21.1 没有公开的 4 角 UV blit 重载,这里用 POSITION_TEX_COLOR 直绘补齐子矩形采样。
        g.flush();
        ...
        com.mojang.blaze3d.vertex.BufferBuilder builder =
                com.mojang.blaze3d.vertex.Tesselator.getInstance().begin(
    ...
    private void drawMapOverlayGrid(GuiGraphics g, PadMapSnapshot map, PadMapProjection.Viewport viewport,
            int clipX, int clipY, int clipW, int clipH) {
    ...
    private void drawPlayerLocation(GuiGraphics g, PadGuiViewState view, PadMapSnapshot map,
    ...
    private void drawPins(GuiGraphics g, PadMapSnapshot map, PadGuiViewState view,
            PadMapProjection.Viewport viewport, int ox, int oy, int w, int h) {
    ...
    private void drawNavigationArrow(GuiGraphics g, int x, int y, float forwardX, float forwardY,
    ...
    private void fillTriangle(GuiGraphics g, float ax, float ay, float bx, float by, float cx, float cy,
            int color) {
    ...
    private void fillClipped(GuiGraphics g, int x, int y, int w, int h, int color, int clipX, int clipY,
            int clipW, int clipH) {
