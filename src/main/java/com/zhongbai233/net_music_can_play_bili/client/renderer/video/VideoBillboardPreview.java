 ...
 * 并在 {@link RenderLevelStageEvent}(AFTER_TRANSLUCENT_BLOCKS)中把纹理提交为世界空间 billboard。正式播放时，
...
public final class VideoBillboardPreview extends VideoBillboardSessionSupport {
    ...
    public static void onRenderFrame(RenderFrameEvent.Pre event) {
        ...
        // YUV core shader 惰性编译必须发生在渲染线程;每帧一次的预热调用是幂等的。
    ...
    public static void onSubmitGeometryAtStage(RenderLevelStageEvent event) {
        ...
        // 1.21.1 没有 SubmitCustomGeometryEvent 提供的共享提交器；本阶段使用自有
        // BufferSource，阶段结束时统一 endBatch 绘制。
        ...
        try {
            ...
            // Iris 会捕获世界几何提交；YUV 兼容模式改走 immediate，避免留下不可见深度状态。
            ...
            if (LEGACY_PREVIEW.requiresProjector()) {
                ...
                for (VideoProjectorBlockEntity projector : projectors) {
                    if (HolographicGlassesClient.shouldHideProjectorVideos()) {
                        submitProjectorPrivacyOverlay(collector, minecraft, camera, projector);
                    } else if (renderYuvFrame) {
                        submitProjectorYuvGeometry(collector, minecraft, camera, projector, yuvTextures);
                    } else {
