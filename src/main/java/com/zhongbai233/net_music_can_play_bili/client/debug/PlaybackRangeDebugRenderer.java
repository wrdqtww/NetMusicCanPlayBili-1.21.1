...
public final class PlaybackRangeDebugRenderer {
    ...
    public static void onSubmitGeometry(RenderLevelStageEvent event) {
        if (!rangeEnabled() || !PortWorldRenderEvents.isStage(event, RenderLevelStageEvent.Stage.AFTER_TRANSLUCENT_BLOCKS)) {
            return;
        }
    ...
    /** 1.21.1 无逐顶点线宽;按线宽分组、每组一个带 LineStateShard 的 RenderType 批次提交。 */
    private static void submitLinesGroupedByWidth(List<DebugLine> lines) {
        if (lines.isEmpty()) {
            return;
        }
        Map<Float, List<DebugLine>> grouped = new TreeMap<>();
        ...
        try {
            ...
            for (Map.Entry<Float, List<DebugLine>> entry : grouped.entrySet()) {
                List<DebugLine> group = entry.getValue();
                collector.submitCustomGeometry(poseStack,
                        PortDebugRenderTypes.translucentLines(entry.getKey()),
                        (pose, buffer) -> {
                            for (DebugLine debugLine : group) {
    ...
    private record DebugLine(float x1, float y1, float z1, float x2, float y2, float z2, float nx, float ny,
            float nz, int color, float width) {
    }
    ...
    public static void onRenderGui(RenderGuiEvent.Post event) {
        ...
        pose.pushPose();
        pose.translate((float) PANEL_MARGIN, (float) PANEL_MARGIN, 0.0F);
        ...
        graphics.drawString(font, "音频播放范围调试", left + PANEL_PADDING, top + 6, TEXT_PRIMARY, false);
    ...
    private static int drawLegend(net.minecraft.client.gui.GuiGraphics graphics,
        ...
        for (LegendItem item : items) {
            ...
            graphics.drawString(font, item.label(), cursorX + 9, cursorY, TEXT_SECONDARY, false);
            cursorX += itemWidth;
        }
        return cursorY + 9;
    }

    private static void drawLifecycleGuide(net.minecraft.client.gui.GuiGraphics graphics,
            net.minecraft.client.gui.Font font, int x, int y, int width) {
        graphics.drawString(font, "重新进入", x, y, TEXT_SECONDARY, false);
        int barX = x + 48;
        int barWidth = Math.max(90, width - 48);
        drawLifecycleSegments(graphics, font, barX, y, barWidth, 3);
    }

    private static void drawEntryCard(net.minecraft.client.gui.GuiGraphics graphics,
        ...
        graphics.drawString(font, entry.state(), x + 7, y + 26, stateColor(entry.state()), false);
    ...
    private static void drawLifecycleSegments(net.minecraft.client.gui.GuiGraphics graphics,
        ...
        for (int index = 0; index < 3; index++) {
            ...
            if (labelWidth + 4 <= segmentWidth) {
                graphics.drawString(font, labels[index], segmentX + (segmentWidth - labelWidth) / 2,
                        y + 1, 0xFF10151D, false);
            }
        }
    }

    private static void drawVolumeMeter(net.minecraft.client.gui.GuiGraphics graphics,
    ...
    private static void drawSyncMeter(net.minecraft.client.gui.GuiGraphics graphics,
    ...
    private static void drawWrapped(net.minecraft.client.gui.GuiGraphics graphics,
    ...
    private static void submitWorld(List<DebugLine> lines, Vec3 cameraPos) {
        int count = 0;
        for (var source : ModernTurntablePlaybackCoordinator.indexedDemandDebugSnapshots()) {
            ...
            drawProfile(lines, cameraPos, center,
                    AudioPlaybackRange.profile(AudioPlaybackRange.DEFAULT_DISTANCE, source.volume(), source.volume()));
            ellipsoid(lines, cameraPos, center, AudioPlaybackRange.SYNC_DISTANCE_BLOCKS,
        ...
        for (IndexedAudioEndpoint endpoint : ClientAudioEndpointIndex.endpointSnapshot()) {
            ...
            drawProfile(lines, cameraPos, new Vec3(endpoint.x(), endpoint.y(), endpoint.z()),
        ...
        for (var media : ClientMediaDemandScheduler.debugSnapshots()) {
            ...
            if (media.headphoneRouted()) {
                cross(lines, cameraPos, media.position(), 1.0F, DIRECT_ROUTE_COLOR, 2.2F);
            } else {
                drawProfile(lines, cameraPos, media.position(),
        ...
        for (var element : ControlConsoleRenderer.elementRangeDebugSnapshots()) {
            ...
            drawProfile(lines, cameraPos, element.center(),
        ...
        for (var console : ControlConsoleRenderer.rangeDebugSnapshots()) {
            ...
            box(lines, cameraPos, console.consolePos().getCenter(), console.radiusX(),
                    console.radiusY(), console.radiusZ(),
                    console.active() ? CONSOLE_ACTIVE_COLOR : CONSOLE_IDLE_COLOR, 1.8F);
        }
    }

    private static void drawProfile(List<DebugLine> lines, Vec3 cameraPos, Vec3 center,
            AudioPlaybackRange.Profile profile) {
        ellipsoid(lines, cameraPos, center, profile.nominalDistance(), profile.nominalDistance(),
                profile.nominalDistance(), NOMINAL_COLOR, 1.8F);
        ellipsoid(lines, cameraPos, center, profile.fadeEndDistance(), profile.fadeEndDistance(),
                profile.fadeEndDistance(), RESOLVE_COLOR, 1.5F);
        ellipsoid(lines, cameraPos, center, profile.noticeDistance(), profile.noticeDistance(),
                profile.noticeDistance(), NOTICE_COLOR, 1.0F);
        cross(lines, cameraPos, center, 0.65F, NOMINAL_COLOR, 2.5F);
    }

    private static void ellipsoid(List<DebugLine> lines, Vec3 cameraPos, Vec3 center,
            double radiusX, double radiusY, double radiusZ, int color, float width) {
        if (!(radiusX > 0.0D && radiusY > 0.0D && radiusZ > 0.0D)) {
            return;
        }
        ring(lines, cameraPos, center, radiusX, radiusY, radiusZ, 0, color, width);
        ring(lines, cameraPos, center, radiusX, radiusY, radiusZ, 1, color, width);
        ring(lines, cameraPos, center, radiusX, radiusY, radiusZ, 2, color, width);
    }

    private static void ring(List<DebugLine> lines, Vec3 cameraPos, Vec3 center,
        ...
        for (int index = 0; index < segments; index++) {
            ...
            line(lines, cameraPos, a, b, color, width);
        }
    }

    private static void box(List<DebugLine> lines, Vec3 cameraPos, Vec3 center,
        ...
        for (int ySign : new int[] { -1, 1 }) {
            for (int zSign : new int[] { -1, 1 }) {
                line(lines, cameraPos, center.add(-halfX, ySign * halfY, zSign * halfZ),
                        center.add(halfX, ySign * halfY, zSign * halfZ), color, width);
            }
        }
        for (int xSign : new int[] { -1, 1 }) {
            for (int zSign : new int[] { -1, 1 }) {
                line(lines, cameraPos, center.add(xSign * halfX, -halfY, zSign * halfZ),
                        center.add(xSign * halfX, halfY, zSign * halfZ), color, width);
            }
            for (int ySign : new int[] { -1, 1 }) {
                line(lines, cameraPos, center.add(xSign * halfX, ySign * halfY, -halfZ),
    ...
    private static void cross(List<DebugLine> lines, Vec3 cameraPos, Vec3 center,
            float radius, int color, float width) {
        line(lines, cameraPos, center.add(-radius, 0, 0), center.add(radius, 0, 0), color, width);
        line(lines, cameraPos, center.add(0, -radius, 0), center.add(0, radius, 0), color, width);
        line(lines, cameraPos, center.add(0, 0, -radius), center.add(0, 0, radius), color, width);
    }

    private static void line(List<DebugLine> lines, Vec3 cameraPos, Vec3 from, Vec3 to,
        ...
        lines.add(new DebugLine(x1, y1, z1, x2, y2, z2, dx * inverse, dy * inverse, dz * inverse, color, width));
    }
