...
public final class VideoPlaybackDebugRenderer {
    ...
    public static void onRenderGui(RenderGuiEvent.Post event) {
        ...
        pose.pushPose();
        pose.translate((float) (screenWidth - PANEL_MARGIN - layout.renderedWidth()), (float) PANEL_MARGIN, 0.0F);
        ...
        graphics.drawString(font, "视频视锥 / 解码调试", left + PANEL_PADDING, top + 6, TEXT_PRIMARY, false);
    ...
    private static void drawLegend(net.minecraft.client.gui.GuiGraphics graphics,
        ...
        for (int index = 0; index < labels.length; index++) {
            ...
            graphics.drawString(font, labels[index], cursor + 9, y, TEXT_SECONDARY, false);
            cursor += itemWidth;
        }
    }

    private static void drawCard(net.minecraft.client.gui.GuiGraphics graphics,
        ...
        graphics.drawString(font, title, x + 7, y + 6, TEXT_PRIMARY, false);
        ...
        for (int stage = 0; stage < 4; stage++) {
            ...
            if (font.width(labels[stage]) + 4 <= segmentWidth) {
                graphics.drawString(font, labels[stage], sx + 2, y + 20, 0xFF10151D, false);
            }
        }
    ...
    private static void drawSyncMeter(net.minecraft.client.gui.GuiGraphics graphics,
        ...
        if (!snapshot.syncActive()) {
            ...
            graphics.drawString(font, "视频同步 离屏暂停", x + 3, y + 1, TEXT_SECONDARY, false);
            return;
        }
        ...
        graphics.drawString(font, label, x + 3, y + 1, TEXT_PRIMARY, false);
    }
    ...
    private static void drawScreen(List<DebugLine> lines, Vec3 cameraPos,
        ...
        cross(lines, cameraPos, center, 0.35D, color, 2.2F);
        horizontalRing(lines, cameraPos, center,
    ...
    private static void horizontalRing(List<DebugLine> lines, Vec3 cameraPos,
        ...
        for (int index = 0; index < 24; index++) {
            ...
            line(lines, cameraPos, center.add(Math.cos(a) * radius, 0, Math.sin(a) * radius),
                    center.add(Math.cos(b) * radius, 0, Math.sin(b) * radius), color, width);
        }
    }

    private static void cross(List<DebugLine> lines, Vec3 cameraPos,
            Vec3 center, double radius, int color, float width) {
    ...
    private static void line(List<DebugLine> lines, Vec3 cameraPos,
