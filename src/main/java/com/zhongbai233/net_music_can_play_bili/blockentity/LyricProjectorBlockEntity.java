...
public class LyricProjectorBlockEntity extends SyncedBlockEntity {
    ...
    protected void loadAdditional(CompoundTag input, HolderLookup.Provider registries) {
        ...
        this.linkedTurntablePos = LinkHelper.loadLinkFromBE(input,
                LINK_KEY + "_has", LINK_KEY + "_x", LINK_KEY + "_y", LINK_KEY + "_z");
        this.projectionYaw = getFloatOr(input, PROJ_YAW, 180.0F);
        this.projectionPitch = getFloatOr(input, PROJ_PITCH, 0.0F);
        this.projectionScale = getFloatOr(input, PROJ_SCALE, 1.0F);
        this.projectionHeight = getFloatOr(input, PROJ_HEIGHT, 1.2F);
        this.projectionDistanceX = getFloatOr(input, PROJ_DISTANCE_X, 0.0F);
        this.projectionDistanceZ = getFloatOr(input, PROJ_DISTANCE_Z, 0.0F);
        this.projectionMode = getIntOr(input, PROJ_MODE, 0);
        this.allowAi = getBooleanOr(input, ALLOW_AI, false);
        syncClientLink();
    }
