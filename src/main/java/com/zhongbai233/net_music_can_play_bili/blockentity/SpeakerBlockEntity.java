...
public class SpeakerBlockEntity extends SyncedBlockEntity {
    ...
    protected void loadAdditional(CompoundTag input, HolderLookup.Provider registries) {
        ...
        this.linkedTurntablePos = LinkHelper.loadLinkFromBE(input,
                LINK_KEY + "_has", LINK_KEY + "_x", LINK_KEY + "_y", LINK_KEY + "_z");
        this.channelIndex = getIntOr(input, CHANNEL_INDEX, CH_NONE);
        this.volume = getFloatOr(input, VOLUME, 1.0f);
        this.autoMixJoc = getBooleanOr(input, AUTO_MIX_JOC, false);
        UUID savedEndpointId = parseUuid(getStringOr(input, ENDPOINT_ID, ""));
        this.endpointId = savedEndpointId != null ? savedEndpointId : UUID.randomUUID();
        this.endpointRevision = Math.max(0L, getLongOr(input, ENDPOINT_REVISION, 0L));
