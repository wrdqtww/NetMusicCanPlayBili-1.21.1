...
public class MP4FocusScreen extends Screen {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (button == 0) {
            if (isBorderDragStart((int) mouseX, (int) mouseY)) {
                startDeviceRotation((int) mouseX, (int) mouseY);
                return true;
            }
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        if (draggingSlider != SliderDrag.NONE) {
            updateSliderDrag((int) mouseX, (int) mouseY);
            return true;
        }
        if (rotatingDevice && button == 0) {
            updateDeviceRotation((int) mouseX, (int) mouseY);
            return true;
        }
        ...
        if (minecraft.getConnection() != null) {
            ...
            if (deviceId == null) {
                PacketDistributor.sendToServer(
                        new com.zhongbai233.net_music_can_play_bili.network.MP4EnsureDeviceIdPacket(
                                MP4FocusState.hand()));
                return;
            }
            MP4Client.cacheFocusedState(deviceId);
            PacketDistributor.sendToServer(new MP4PlaybackControlPacket(action, MP4FocusState.selectedQueueIndex(),
