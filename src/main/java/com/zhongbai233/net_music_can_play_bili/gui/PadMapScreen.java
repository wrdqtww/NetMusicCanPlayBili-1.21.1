...
public class PadMapScreen extends Screen {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (mouseX >= closeX && mouseX <= closeX + CLOSE_SIZE && mouseY >= closeY
    ...
    private void updateMapTexture(PadMapSnapshot current) {
        ...
        if (mapTexture == null || mapTexture.getPixels() == null) {
            return;
        }
        ...
        for (int z = 0; z < height; z++) {
            for (int x = 0; x < width; x++) {
                ...
                image.setPixelRGBA(x, z, tile == null
                        ? net.minecraft.util.FastColor.ABGR32.fromArgb32(PadMapTileKind.UNKNOWN.color())
                        : net.minecraft.util.FastColor.ABGR32.fromArgb32(tile.color()));
            }
        }
        mapTexture.upload();
    ...
    private void ensureMapTexture(int width, int height) {
        ...
        releaseMapTexture();
        mapTextureId = ResourceLocation.fromNamespaceAndPath("net_music_can_play_bili",
                "dynamic/pad_map_screen_" + hand.name().toLowerCase(java.util.Locale.ROOT));
        mapTexture = new DynamicTexture(width, height, false);
    ...
    private void drawPanel(GuiGraphics g, int x, int y, int mouseX, int mouseY) {
        ...
        g.drawCenteredString(font, getTitle(), x + PANEL_W / 2, y + 8, 0xFFBDE7FF);
        int closeX = x + PANEL_W - CLOSE_SIZE - 7;
        int closeY = y + (HEADER_H - CLOSE_SIZE) / 2;
        boolean hovered = mouseX >= closeX && mouseX <= closeX + CLOSE_SIZE && mouseY >= closeY
                && mouseY <= closeY + CLOSE_SIZE;
        g.drawCenteredString(font, Component.literal("✕"), closeX + CLOSE_SIZE / 2, closeY + 4,
    ...
    private void drawMap(GuiGraphics g, int x, int y, int w, int h) {
        ...
        if (snapshot == null) {
            g.drawCenteredString(font, Component.literal("Loading map..."), x + w / 2, y + h / 2 - 4, 0xFFB9C7D7);
            return;
        }
        ...
        if (mapTextureId != null) {
            com.zhongbai233.net_music_can_play_bili.port.shim.PortGuiTextures.blitRegion(g, mapTextureId,
                    visualOriginX, visualOriginY, visualOriginX + drawW, visualOriginY + drawH,
                    0.0F, 0.0F, 1.0F, 1.0F);
        }
    ...
    private void drawTriggerPins(GuiGraphics g, int originX, int originY, float cellPx) {
        ...
        for (PadTriggerPoint point : document.triggerPoints()) {
            ...
            g.drawCenteredString(font, Component.literal(String.valueOf(index++)), px, pz - 18, 0xFFFFFFFF);
        }
    }
    ...
    private void drawSidebar(GuiGraphics g, int x, int y, int w, int h) {
        ...
        g.drawString(font, Component.literal("地图"), x + 8, y + 10, 0xFFBDE7FF);
        g.drawString(font, Component.literal("缩放 x" + String.format(java.util.Locale.ROOT, "%.1f", zoom)), x + 8,
                y + 26, 0xFF8EA0B5);
        g.drawString(font, Component.literal("媒体 " + document.mediaEntries().size()), x + 8, y + 48, 0xFFE8E0C8);
        g.drawString(font, Component.literal("点位 " + document.triggerPoints().size()), x + 8, y + 62, 0xFFFFC857);
        g.drawString(font, Component.literal(document.locked() ? "已锁定" : "草稿"), x + 8, y + 84,
                document.locked() ? 0xFFFFC857 : 0xFF6FE28A);
        g.drawString(font, Component.literal("拖拽平移"), x + 8, y + h - 42, 0xFF7E8EA3);
        g.drawString(font, Component.literal("滚轮缩放"), x + 8, y + h - 28, 0xFF7E8EA3);
        g.drawString(font, Component.literal("点位播放→Pad"), x + 8, y + h - 14, 0xFF6FD2FF);
    }
