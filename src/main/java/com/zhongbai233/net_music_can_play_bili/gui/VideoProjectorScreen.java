...
public class VideoProjectorScreen extends BlackGoldScreen {
    ...
    private void sendConfigToServer(VideoProjectorBlockEntity be) {
        if (minecraft != null && minecraft.getConnection() != null) {
            PacketDistributor.sendToServer(new VideoProjectorConfigPacket(
                    blockPos,
    ...
    private void drawLinkInfo(GuiGraphics g, int bx, int by) {
        ...
        if (linked == null) {
            g.drawCenteredString(font, Component.literal("未连接"), cx, iy, TEXT_DIM);
            return;
        }
        ...
        g.drawCenteredString(font, Component.literal(info), cx, iy, GOLD);
        ...
        g.drawCenteredString(font, Component.literal(playbackInfo), cx, iy + 12, TEXT_DIM);
        if (status.active() && status.actualQuality() > 0) {
            ...
            g.drawCenteredString(font, Component.literal(actualInfo), cx, iy + 24, TEXT_DIM);
        }
    }
    ...
    private void drawLabels(GuiGraphics g, int bx, int by) {
        ...
        for (String lb : labels) {
            g.drawCenteredString(font, Component.literal(lb), lx + 27, ry, TEXT_SECONDARY);
            ry += 26;
        }
        g.drawCenteredString(font, Component.literal("实际清晰度会受视频源和账号权限影响"),
    ...
    private void drawQualityOverlay(GuiGraphics g) {
        ...
        g.drawCenteredString(font, Component.literal("◈ " + qualityBtn.currentOption()),
                x + w / 2, y + 5, GOLD);
    }
