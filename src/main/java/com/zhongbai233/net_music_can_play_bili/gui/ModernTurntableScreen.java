...
public class ModernTurntableScreen extends BlackGoldScreen {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (mouseX >= cx && mouseX <= cx + CLOSE_SIZE
    ...
    protected void drawHeader(GuiGraphics g, int bx, int by, int mx, int my) {
        ...
        g.drawCenteredString(font, getTitle(), bx + T_BOX_W / 2, by + 9, GOLD);
    ...
    private void drawSongInfo(GuiGraphics g, int bx, int by, ModernTurntableBlockEntity t) {
        ...
        g.drawCenteredString(font, Component.literal(displayName), cx, iy, songColor);

        String fmtLabel = cachedFormatLabel;
        if (fmtLabel.isEmpty() && t != null && t.hasDisc())
            fmtLabel = "CD 唱片";
        if (!fmtLabel.isEmpty())
            g.drawCenteredString(font, Component.literal(fmtLabel), cx, iy + 14, TEXT_SECONDARY);
    }
    ...
    private void drawProgressOverlay(GuiGraphics g, int bx, int by) {
        ...
        g.drawString(font, Component.literal(label), x + (w - lw) / 2, y + h + 1, TEXT_SECONDARY);
    }
    ...
    private void send(ModernTurntableControlPacket.Action action, long targetMillis) {
        if (Minecraft.getInstance().getConnection() != null)
            PacketDistributor.sendToServer(
                    new ModernTurntableControlPacket(blockPos, action, Math.max(0L, targetMillis)));
    }
