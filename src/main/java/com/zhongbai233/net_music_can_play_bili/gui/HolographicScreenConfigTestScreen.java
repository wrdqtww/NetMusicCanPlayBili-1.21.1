...
public class HolographicScreenConfigTestScreen extends HolographicEditorInputScreen {
    ...
    protected void drawNumericPanel(GuiGraphics g, int mouseX, int mouseY) {
        ...
        g.drawString(font, Component.literal("屏幕属性"), px + 8, py + 5, GOLD);
        g.drawString(font, Component.literal("数值 / 预设"), px + pw - 68, py + 5, TEXT_DIM);
        ...
        for (int i = 0; i < 3; i++) {
            g.drawString(font, Component.literal(labels1[i]), px + 6 + cellW * i, row1y + 5, TEXT_SECONDARY);
            g.drawString(font, Component.literal(labels2[i]), px + 6 + cellW * i, row2y + 5, TEXT_SECONDARY);
        }
        int presetX1 = px + 6 + cellW * 3 + 6;
        g.drawString(font, Component.literal("预设"), presetX1, row1y - 3, TEXT_DIM);
    }
