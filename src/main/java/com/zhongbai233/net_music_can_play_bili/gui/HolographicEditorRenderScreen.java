...
abstract class HolographicEditorRenderScreen extends HolographicConsoleInspectorScreen {
    ...
    protected void drawControlConsolePanels(GuiGraphics g, int mouseX, int mouseY) {
        ...
        g.drawString(font, Component.literal("元素层级"), 10, 10, GOLD);
        g.drawString(font, Component.literal("场景元素"), 10, 21, TEXT_DIM);

        int x = rightX + 12;
        g.drawString(font, Component.literal("检查器"), x, 10, GOLD);
        ControlConsoleDocument document = currentConsoleDocument();
        PreviewScreenSpec screen = selectedScreenOrNull();
        if (screen != null) {
            g.drawString(font, Component.literal(BlackGoldUi.ellipsize(font,
            ...
            g.drawString(font, Component.literal(typeHint), x, 68, TEXT_DIM);
            g.drawString(font, Component.literal("位置 / 尺寸 / 旋转 / 高级变换"), x, 82, TEXT_DIM);
            String[][] labels = { { "距离", "位置X" }, { "位置Y", "高度" },
                    { "比例", "Yaw" }, { "Pitch", "Roll" } };
            for (int row = 0; row < labels.length; row++) {
                g.drawString(font, Component.literal(labels[row][0]), rightX + 12, 117 + row * 22, TEXT_SECONDARY);
                g.drawString(font, Component.literal(labels[row][1]), rightX + 122, 117 + row * 22, TEXT_SECONDARY);
            }
            if (showTransformInspector) {
                String[][] advancedLabels = { { "缩放X", "缩放Y" }, { "缩放Z", "枢轴X" },
                        { "枢轴Y", "枢轴Z" }, { "X←Y", "Y←X" } };
                for (int row = 0; row < advancedLabels.length; row++) {
                    g.drawString(font, Component.literal(advancedLabels[row][0]), rightX + 12, 211 + row * 22,
                            TEXT_SECONDARY);
                    g.drawString(font, Component.literal(advancedLabels[row][1]), rightX + 122, 211 + row * 22,
                            TEXT_SECONDARY);
                }
                g.drawString(font, Component.literal(screen.type == ElementType.AUDIO
                        ? "音源首版忽略 scale / pivot / skew" : "完整仿射变换"), x, 288, TEXT_DIM);
            } else {
                g.drawString(font, Component.literal("元素内容"), x, 194, TEXT_DIM);
            }
        } else if (document != null) {
            g.drawString(font, Component.literal("中控台 / Console"), x, 34, TEXT_SECONDARY);
            g.drawString(font, Component.literal("名称"), x, 67, TEXT_SECONDARY);
            g.drawString(font, Component.literal(document.hasSourceBinding() ? "媒体源：已绑定" : "媒体源：未绑定"),
                    x, 86, document.hasSourceBinding() ? 0xFF75D6A0 : TEXT_DIM);
            g.drawString(font, Component.literal("硬范围 X / Y / Z"), x, 96, TEXT_DIM);
            g.drawString(font, Component.literal("访问权限"), x, 138, TEXT_DIM);
            g.drawString(font, Component.literal("文档 Rev：" + document.revision()), x, 178, TEXT_SECONDARY);
            g.drawString(font, Component.literal("所有者：" + (document.ownerId() != null
                    ? document.ownerId().toString().substring(0, 8) : "未认领")), x, 190, TEXT_DIM);
            g.drawString(font, Component.literal("可信玩家 UUID（逗号分隔）"), x, 204, TEXT_DIM);
        }
            if (!consoleSaveStatus.isEmpty()) {
                int statusColor = consoleSaveConflict ? 0xFFFF8A75 : TEXT_SECONDARY;
                g.drawString(font, Component.literal(BlackGoldUi.ellipsize(font, consoleSaveStatus,
    ...
    protected void drawPreview(GuiGraphics g, int x, int y, int w, int h, int mouseX, int mouseY) {
        ...
        if (!firstPersonPreview && controlConsolePos != null) {
            g.drawString(font, Component.literal(BlackGoldUi.ellipsize(font, controlConsoleSourceLabel(),
        ...
        if (playerHovered && !controlConsoleMode) {
            g.drawCenteredString(font, Component.literal("点击进入第一人称"), x + w / 2, y + 31, 0xFFBFF7FF);
        }
    ...
    protected void drawEditorHud(GuiGraphics g) {
        ...
        outline(g, barX, 2, barW, ICON_H + 4, 0x8045E7FF);
        int activeIndex = activeTool.ordinal();
        int ax = barX + 2 + activeIndex * (ICON_W + ICON_GAP);
        if (!navigationMode()) {
            outline(g, ax - 1, 3, ICON_W + 2, ICON_H + 2, 0xFF45E7FF);
        }
        ...
        g.drawString(font, Component.literal(BlackGoldUi.ellipsize(font, toolTip, hudWidth)), hudX, height - 34,
            TEXT_SECONDARY);
    ...
    protected void drawOrientationAxis(GuiGraphics g, int centerX, int centerY, OrientationAxis axis,
            boolean front) {
        ...
        if (!axis.label.isEmpty()) {
            ...
            g.drawCenteredString(font, Component.literal(axis.label), endX, endY - 3, labelColor);
        }
    }
    ...
    protected void submitPipPreview(GuiGraphics g, int x, int y, int w, int h, int mouseX, int mouseY,
        ...
        if (controlConsoleMode && minecraft.level != null && controlConsolePos != null && terrainDocument != null) {
            TerrainBounds terrainBounds = com.zhongbai233.net_music_can_play_bili.client.terrain.TerrainHardRangeBounds
                    .around(controlConsolePos.getX(), controlConsolePos.getY(), controlConsolePos.getZ(),
                        ...
                        terrainDocument.hardRangeZ(), minecraft.level.getMinBuildHeight(), minecraft.level.getMaxBuildHeight());
        ...
        HolographicPreviewPipRenderState pipState = new HolographicPreviewPipRenderState(player,
        ...
        // 1.21.1 无 GUI 图集 PIP:每帧把场景直接离屏渲染进私有 Framebuffer,再把颜色纹理贴回预览矩形。
    ...
    protected abstract void drawNumericPanel(GuiGraphics g, int mouseX, int mouseY);
