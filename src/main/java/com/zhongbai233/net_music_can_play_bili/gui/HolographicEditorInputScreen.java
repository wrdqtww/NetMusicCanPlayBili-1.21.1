...
abstract class HolographicEditorInputScreen extends HolographicEditorRenderScreen {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (isCloseButton(mouseX, mouseY)) {
            onClose();
            return true;
        }
        ...
        if ((button == 0 || button == 1) && inPreview(mouseX, mouseY)) {
            int pipX = previewX() + 1;
            int pipY = previewY() + 26;
            int pipW = previewW() - 2;
            int pipH = previewH() - 54;
            CameraFrame cameraFrame = orbitCameraFrameFor(pipX, pipY, pipW, pipH);
            activeHandle = firstPersonPreview || button == 1 ? GizmoHandle.NONE
                    : gizmoHandleAt(mouseX, mouseY, pipX, pipY, pipW, pipH, cameraFrame);
            SceneHit sceneHit = firstPersonPreview ? SceneHit.player(0.0D)
                    : sceneHitAt(mouseX, mouseY, cameraFrame);
            previewClickHitPlayer = button == 0 && activeHandle == GizmoHandle.NONE
                    && sceneHit.type == SceneHitType.PLAYER && !controlConsoleMode;
            if (!firstPersonPreview && button == 0 && activeHandle == GizmoHandle.NONE
            ...
            previewDragStartedWithoutElement = controlConsoleMode && button == 0
                    && activeHandle == GizmoHandle.NONE
                    && sceneHit.type == SceneHitType.NONE;
            draggingPreview = true;
            previewDragButton = button;
            dragMode = switch (EditorMouseDragPolicy.action(button, firstPersonPreview,
                    activeHandle != GizmoHandle.NONE)) {
            ...
            if (dragMode == DragMode.GIZMO) {
                gizmoDragSession = createGizmoDragSession(mouseX, mouseY, cameraFrame, activeHandle);
            ...
            previewClickX = mouseX;
            previewClickY = mouseY;
            lastMouseX = mouseX;
            lastMouseY = mouseY;
            return true;
        }
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        if (draggingPreview && button == previewDragButton) {
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) {
        if (draggingPreview && button == previewDragButton) {
            double dx = mouseX - previewClickX;
            double dy = mouseY - previewClickY;
            if (BlankClickSelectionPolicy.shouldDeselect(previewDragStartedWithoutElement, button,
                    previewClickX, previewClickY, mouseX, mouseY) && selectedScreen >= 0) {
                enterNavigationMode();
                init();
            }
            if (button == 0 && dragMode != DragMode.GIZMO && dx * dx + dy * dy < 16.0D
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (controlConsoleMode && keyCode == GLFW.GLFW_KEY_V) {
            startWorldRoaming();
            return true;
        }
        if (setFlyKey(keyCode, true)) {
            return true;
        }
