...
public class WhitelistReviewScreen extends Screen {
    ...
    private void drawHeader(GuiGraphics g, int bx, int by, int mx, int my) {
        ...
        g.drawString(font, Component.literal(BlackGoldUi.ellipsize(font, pageStatus(), boxWidth() - 28)),
        ...
        g.drawCenteredString(font, Component.literal("✕"), cx + CLOSE_SIZE / 2, cy + 4,
    ...
    private void drawList(GuiGraphics g, int bx, int by, int mx, int my) {
        ...
        if (itemCount == 0) {
            g.drawCenteredString(font, Component.literal(historyTab ? "暂无移除历史" : "当前白名单为空"),
        ...
        for (int i = 0; i < rows; i++) {
            ...
            g.drawString(font, Component.literal(BlackGoldUi.ellipsize(font, label, boxWidth() - 142)),
                    x + 6, y + 6, selected ? BlackGoldUi.GOLD : BlackGoldUi.TEXT_PRIMARY, false);
            int authorX = bx + boxWidth() - 116;
            g.drawString(font, Component.literal(BlackGoldUi.ellipsize(font, byName, 90)),
    ...
    private void drawDetails(GuiGraphics g, int bx, int by) {
        ...
        if (historyTab) {
            ...
            if (record == null) {
                g.drawString(font, Component.literal("选择一条历史记录查看详情"), x + 8, y + 10, BlackGoldUi.TEXT_DIM, false);
                return;
            }
            ...
            g.drawString(font, Component.literal(BlackGoldUi.ellipsize(font, "备注：" + record.note(), textWidth)),
        ...
        if (entry == null) {
            g.drawString(font, Component.literal("选择一个条目查看详情"), x + 8, y + 10, BlackGoldUi.TEXT_DIM, false);
            return;
        }
        g.drawString(font, Component.literal(BlackGoldUi.ellipsize(font, "资源：" + entry.id(), textWidth)),
        ...
        if (!comments.isEmpty() && maxComments > 0) {
            g.drawString(font, Component.literal("最近评论："), x + 8, y + line, BlackGoldUi.GOLD, false);
            line += 13;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        int itemCount = historyTab ? removalRecords.size() : entries.size();
        if (button == 0 && itemCount > visibleRows() && inScrollbar(mouseX, mouseY, bx, by)) {
            int thumbY = scrollbarThumbY(by);
            int thumbHeight = scrollbarThumbHeight();
            if (mouseY < thumbY || mouseY > thumbY + thumbHeight) {
                scrollbarDragOffsetY = thumbHeight / 2.0D;
                scrollToScrollbarPosition(mouseY, by);
            } else {
        ...
        if (mouseX >= listX && mouseX <= listX + boxWidth() - 28
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dragX, double dragY) {
        if (draggingScrollbar && button == 0) {
            scrollToScrollbarPosition(mouseY, boxY());
            return true;
        }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (promptMode != PromptMode.NONE) {
            ...
            if ((keyCode == 257 || keyCode == 335) && getFocused() == promptField) {
                submitPrompt();
                return true;
            }
            return super.keyPressed(keyCode, scanCode, modifiers);
    ...
    private void previewSelected() {
        ...
        interactionStatus = "";
        rebuildButtons();
        PacketDistributor.sendToServer(new WhitelistReviewActionPacket(
    ...
    private void request(WhitelistReviewActionPacket.Action action, String value) {
        PacketDistributor.sendToServer(new WhitelistReviewActionPacket(action, value));
    }
    ...
    private void submitPrompt() {
        ...
        interactionStatus = "";
        rebuildButtons();
        PacketDistributor.sendToServer(new WhitelistReviewActionPacket(action, promptTargetId,
