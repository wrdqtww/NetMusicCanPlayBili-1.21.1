...
public class WhitelistPreviewScreen extends Screen {
    ...
    private void drawHeader(GuiGraphics g, int bx, int by, int mx, int my) {
        ...
        g.drawString(font, Component.literal(BlackGoldUi.ellipsize(font, payload.title(), boxWidth() - 54)),
    ...
    private void drawVideo(GuiGraphics g, int bx, int by) {
        ...
        if (!hasVideo()) {
            g.drawCenteredString(font, Component.literal("纯音频预览"), x + videoWidth() / 2, y + videoHeight() / 2 - 14,
                    BlackGoldUi.GOLD);
            g.drawCenteredString(font, Component.literal("此条目仅播放音频"),
        ...
        if (frame.hasFrame() && !frame.yuv() && frame.rgbaTexture() != null) {
            com.zhongbai233.net_music_can_play_bili.port.shim.PortGuiTextures.blitRegion(g, frame.rgbaTexture(), x, y, x + videoWidth(), y + videoHeight(), 0.0F, 1.0F, 0.0F, 1.0F);
            if (VideoBillboardPreview.hasNetworkFailure(sessionId)) {
                g.drawCenteredString(font, Component.literal("点击画面重新连接"), x + videoWidth() / 2,
                        y + videoHeight() - 20, 0xFFFFD166);
            }
            return;
        }
        if (videoResolveFailed) {
            g.drawCenteredString(font, Component.literal(videoResolveNetworkFailure
                    ? "视频网络连接失败"
        ...
        String text = frame.hasFrame() && frame.yuv()
                ? "正在准备视频画面..."
                : "正在加载视频画面...";
        g.drawCenteredString(font, Component.literal(text), x + videoWidth() / 2, y + videoHeight() / 2 - 6, BlackGoldUi.TEXT_DIM);
    }
    ...
    private void drawProgress(GuiGraphics g, int bx, int by, int mx, int my) {
        ...
        g.drawString(font, Component.literal(timeText(displayMillis()) + " / " + totalText),
    ...
    private void drawFooter(GuiGraphics g, int bx, int by) {
        ...
        g.drawString(font, Component.literal(BlackGoldUi.ellipsize(font, footer, boxWidth() - 32)),
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        String sessionId = WhitelistPreviewPacket.sessionId(payload.previewId(), payload.elapsedMillis());
        if (hitVideo(bx, by, mouseX, mouseY) && videoResolveFailed) {
        ...
        if (hitVideo(bx, by, mouseX, mouseY) && VideoBillboardPreview.retryNetworkFailure(sessionId)) {
            return true;
        }
        if (hitProgress(bx, by, mouseX, mouseY)) {
            scrubbing = true;
            updateScrub(bx, mouseX);
            return true;
        }
        return super.mouseClicked(mouseX, mouseY, button);
    }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (promptingDelete) {
            ...
            if ((keyCode == 257 || keyCode == 335) && getFocused() == deleteNoteField) {
                confirmDelete();
                return true;
            }
