...
public class LiveStreamerScreen extends BlackGoldScreen {
    ...
    private void sendVolume() {
        ...
        PacketDistributor.sendToServer(new LiveStreamerControlPacket(blockPos,
    ...
    private void sendControl(LiveStreamerControlPacket.Action action) {
        ...
        PacketDistributor.sendToServer(new LiveStreamerControlPacket(blockPos, action, roomInput, 0));
    }
    ...
    protected void drawContent(GuiGraphics g, int bx, int by, int mx, int my) {
        int cx = bx + BOX_W / 2;
        g.drawCenteredString(font, Component.translatable("gui.net_music_can_play_bili.live_streamer.room_label"),
        ...
        g.drawCenteredString(font, status, cx, by + boxH() - 22, color);
    }
