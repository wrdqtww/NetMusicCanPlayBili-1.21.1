...
public final class MediaBindingData {
    ...
    public static MediaSource readSource(CompoundTag tag) {
        ...
        SourceKind kind = SourceKind.byName(tag.getString(TAG_SOURCE_KIND));
        if (kind == SourceKind.MP4 || kind == SourceKind.PAD) {
            UUID deviceId = parseUuid(tag.getString(TAG_MEDIA_DEVICE_ID));
            if (deviceId == null) {
                deviceId = parseUuid(tag.getString(TAG_MP4_DEVICE_ID));
            }
            if (deviceId == null) {
                return null;
            }
        ...
        if (kind == SourceKind.VIDEO_PROJECTOR || kind == SourceKind.TURNTABLE) {
            ResourceKey<Level> dimension = parseDimension(tag.getString(TAG_DIMENSION));
            if (dimension == null) {
                return null;
            }
