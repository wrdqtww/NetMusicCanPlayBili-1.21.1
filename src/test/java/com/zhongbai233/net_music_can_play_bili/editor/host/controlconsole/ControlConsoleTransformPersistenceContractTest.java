...
/** 纯 Java 测试源集不加载 Minecraft；这里锁定 NBT/网络接线不能漏掉任一 v7 变换或亮度分量。 */
class ControlConsoleTransformPersistenceContractTest {
    @Test
    void schemaV7TransformAndBrightnessArePresentInNbtAndNetworkBothDirections() throws Exception {
        ...
        for (String accessor : new String[] { "scaleX()", "scaleY()", "scaleZ()", "pivotX()", "pivotY()",
                "pivotZ()", "skewXByY()", "skewYByX()", "brightness()" }) {
            assertTrue(packet.contains("element." + accessor), () -> "missing packet write for " + accessor);
        }
        assertTrue(occurrences(packet, "buf.readFloat(), buf.readFloat(), buf.readFloat()") >= 3,
                "packet read must consume all 9 v7 transform and brightness floats");
    }
